...
class UserManager(BaseUserManager):
    ...
    def _create_user(self, email, password, **extra_fields):
        if not email:
            raise ValueError('Указанный адрес электронной почты должен быть указан')
...
class User(AbstractUser):
    ...
    is_active = models.BooleanField(
        _('active'),
        default=False,
        help_text=_('Активен ли пользователь''Снимите этот флажок вместо удаления учётных записей'),
    )
...
class ConfirmEmailToken(models.Model):
    ...
    user = models.ForeignKey(User, verbose_name=_("The token owner"), related_name='confirm_email_tokens',
                             blank=True, on_delete=models.CASCADE)
    created_at = models.DateTimeField(auto_now_add=True, verbose_name=_("Token creation date"))
    key = models.CharField(verbose_name = _('Key'), max_length=64, db_index=True, unique=True)
    ...
    def __str__(self):
        return 'Токен сброса пароля для {user}'.format(user=self.user)
